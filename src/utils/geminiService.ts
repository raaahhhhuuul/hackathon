...
export class GeminiService {
  ...
  async generateAnalyticsInsights(data: any, query: string): Promise<string> {
    try {
...
Please provide a comprehensive analysis with specific insights and actionable recommendations.`;
  ...
  async generateBusinessInsights(data: any): Promise<string> {
    try {
...
Format your response in a clear, structured manner.`;
  ...
  async generateChatbotResponse(userMessage: string, contextData?: any): Promise<string> {
    try {
...
${JSON.stringify(contextData, null, 2)}`;
  ...
  async generateTrendAnalysis(historicalData: any): Promise<string> {
    try {
      const prompt = `${SYSTEM_PROMPTS.analytics}
